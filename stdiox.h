...
int getkey(int block) {
  int key;
  if (termin_keystart == termin_keyend) { //empty ring buffer
    if (block) nodelay(stdscr, FALSE); else nodelay(stdscr, TRUE);
    key = getch();
    //if key is mouse event add to mouse buffer
  }
  else {
    key = termin_keybuf[termin_keystart++];
    if (termin_keystart > 1000) termin_keystart = 0;
  }
  if (key == ERR) key = 0;
  return key;
}
...
void colorx(int fg, int bg) {
  ...
  if (fg & 16) { //is bit 5 set?
    blink = 1;
    fg = fg & (0xff - 16); //turn off bit 4
  }
  if (fg & 8) { //is bit 4 set?
    bright = 1;
    fg = fg & (0xff - 8); //turn off bit 3
  }

  if (bright) attroff(A_DIM); else attron(A_DIM);
  if (blink) attron(A_BLINK); else attroff(A_BLINK);
  
  if (fg == 0 && bg == 0) { //special case
